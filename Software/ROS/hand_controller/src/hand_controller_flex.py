...
goal_pos = 0;
pub = rospy.Publisher('/flex_controller/corosmmand', Float64)

def transform_callback(data):
    global goal_pos
    rospy.loginfo(': Adduction current motor angle {0}'.format(data.current_pos))
...
def dxl_control():
    ...
    rospy.Subscriber('/flex_controller/state', JointState, transform_callback)
    # Initial movement.
